...
const btn=document.querySelector("form #result");
...
for(let select of dropdown){
    for(let currCode in countryList){
        ...
        if(selectName=="from"&&currCode=="USD"){
            newOption.selected="selected";
        }
        if(selectName=="to"&&currCode=="INR"){
            newOption.selected="selected";
        }
...
btnicon.addEventListener("click",(evt)=>{
    ...
    // Recalculate result
    updateRate();
    btnicon.classList.add("swapp");
});



let updateFalg=(element)=>{
    let currCode=element.value;
    let flag=countryList[currCode];
    let newFlag=`https://flagsapi.com/${flag}/flat/64.png`;
...
let updateRate=async ()=>{
    ...
    let object=data[from];
    let rate;
    for(let too in object){
        if(too==to){
            rate=object[too];
            break;
        }
